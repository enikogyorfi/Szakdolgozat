...
\node[
  ...
  align=center
] (crime) at (0,0) {Bűnözési\\gyakoriság};
...
\node[
  ...
  align=center
] (time) at (0,1.75) {időbeli\\változás};
...
\node[
  ...
  minimum height=0.75cm,
  align=center
] (space) at (-4.2,0) {térbeli\\különbségek};
...
\node[
  ...
  minimum height=0.75cm,
  align=center
] (society) at (4.2,0) {társadalmi\\környezet};
...
\node[
  ...
  minimum height=0.75cm,
  align=center
] (random) at (0,-1.75) {véletlen\\ingadozás};

% Nyilak
\draw[->,thick] (time.south) -- (crime.north);
\draw[->,thick] (space.east) -- (crime.west);
\draw[->,thick] (society.west) -- (crime.east);
\draw[->,thick] (random.north) -- (crime.south);
...
\node[
  ...
  \textbf{Modellezési kérdés}\\[0.08cm]
...
\includegraphics[
  ...
  keepaspectratio
]{major_crimes_chicago_2.png}

\vspace{0.15cm}
{\small A leggyakoribb bűncselekménytípusok megoszlása.}
\end{column}
...
\begin{block}{Bűnözési adatok}
\begin{itemize}
  \setlength\itemsep{0.08cm}
  \item Chicago nyilvános bűnözési adatbázisa
  \item időszak: 2001--2025
  \item több millió rekord
  \item több szintű térbeli felbontás
\end{itemize}
\end{block}

\vspace{0.14cm}

\begin{block}{Demográfiai adatok}
\begin{itemize}
  \setlength\itemsep{0.08cm}
  \item American Community Survey adatok
  \item Community Area szintű felbontás
  \item éves adatok
  \item társadalmi-demográfiai jellemzők
\end{itemize}
\end{block}
...
\begin{itemize}
  ...
  \item csak a legutóbbi rövid távú bűnözési szintből indul ki.
\end{itemize}
\end{block}
...
% Az onlytextwidth biztosítja, hogy a dia két szélén megmaradjon a standard beamer margó
...
\begin{column}{0.48\textwidth}
  ...
  \vspace{0.1cm} % Kis térköz a blokk és a felsorolás közé, hogy szellősebb legyen

  % A leftmargini lokális átállításával a pöttyök egy vonalba kerülnek a fenti blokk bal szélével
  \begingroup
  \setlength{\leftmargini}{1.2em}
  \begin{itemize}
    \item $\mu_{i,t}=\log(S_{i,t+1}/S_{i,t})$
    \item $\mu_i=\overline{\mu_{i,t}}+0.5\cdot \operatorname{sd}(\mu_{i,t})^2$: drift.
    \item $\sigma_i=\operatorname{sd}(\mu_{i,t})$: volatilitás.
...
\begin{column}{0.48\textwidth} % Szintén 0.48, hogy szimmetrikus legyen
  \centering
  \vspace{0.2cm} % Kicsit lejjebb toljuk, hogy egyvonalban legyen a bal oldali tartalommal
  \begin{tikzpicture}[every node/.style={font=\scriptsize}]
...
\vspace{-0.1cm} % Helykihagyás az alsó fókuszblokk előtt

\begin{center}
  \begin{minipage}{0.94\textwidth}
    \begin{block}{Lényeg}
      A GBM nem determinisztikus predikció, hanem lehetséges jövőbeli pályák szimulációja.
    \end{block}
  \end{minipage}
\end{center}
\end{frame}
...
\begin{itemize}
  ...
  \item Időbeli és szezonális feature-ök,
...
\end{frame}\begin{frame}{Random Forest modellek összehasonlítása}
...
A térbeli mintázatok jól látszanak, a rolling feature-ös modell pontosabb.
\end{block}
\end{minipage}
\end{center}
...
% Az [onlytextwidth] továbbra is kell, de most az oszlopok szélessége tudatosan NEM éri el az 1.0-t.
% Bal oszlop (0.54) + Jobb oszlop (0.32) = 0.86. A maradék 0.14\textwidth (14% szabad hely)
% úgy lesz elosztva, hogy a jobb oldalon jelentős üres margó keletkezik.
\begin{columns}[T, onlytextwidth]
    \begin{column}{0.0001\textwidth}
    % Szándékosan üresen hagyva a jobb oldali nagy margóért
  \end{column}
  % BAL OLDAL (Táblázat és Grafikon) - kicsit keskenyebb lett, hogy beljebb húzódjon
  \begin{column}{0.55\textwidth}
    ...
    \begin{axis}[
      ybar,
      width=1.0\textwidth, % Továbbra is pontosan követi a felette lévő táblázat szélét
      height=3.8cm,
      ymin=88,
      ymax=94,
      ylabel={Accuracy (\%)},
      ...
      bar width=12pt, % Kicsit keskenyebb oszlopok, hogy illeszkedjen a kisebb szélességhez
      grid=major,
      grid style={gray!20},
    ]
  ...
  % Belső rugalmas térköz a két oszlop között (vízszintes rugó)
  \hfill

  % JOBB OLDAL (Blokkok/Üzenetek) - szintén keskenyebb és beljebb van
  \begin{column}{0.32\textwidth}
    ...
    \small
    \centering
    Random Forest mozgó változókkal kiegészítve\
    ...
    \small
    \centering
    A mozgó változók jobban megragadták a rövid távú mintázatokat.
    \end{block}

  \end{column}

  % EXTRA ÜRES OSZLOP A JOBB SZÉLEN: ez kényszeríti a szöveget, hogy ne menjen ki a dia szélére
...
\includegraphics[
  ...
  clip
]{socio_feature_relative_importance_heatmap_ppt.png}
\end{column}
...
\begin{frame}{A gyakorlati rész fő tanulságai}

\begin{columns}[c,onlytextwidth]

% BAL OLDAL: rövidebb ábra
\begin{column}{0.38\textwidth}
\centering
\begin{tikzpicture}[every node/.style={font=\scriptsize}, >=Latex]

\node[
  draw=deepblue, very thick, rounded corners,
  fill=softblue!12,
...
\node[
  ...
  align=center
] (concl) at (0,-3.8) {Előrejelzés\\és értelmezés};

\draw[->,thick] (gbm) -- (rf);
\draw[->,thick] (rf) -- (demo);
\draw[->,thick] (demo) -- (concl);

\end{tikzpicture}
\end{column}

% JOBB OLDAL: rövidebb szöveg
\begin{column}{0.58\textwidth}

\begin{block}{Fő megállapítások}
\begin{itemize}
  \item \textbf{GBM:} több lehetséges jövőbeli pályát szimulált, így a bizonytalanságot is szemléltette.
  \item \textbf{RF lag:} a rövid távú múltbeli információ bizonyult fontosnak.
  \item \textbf{RF rolling:} a legjobb eredményt adta MAPE és Accuracy alapján.
  \item \textbf{Demográfia:} kiegészítő magyarázó erőt adott, főként az értelmezésben.
\end{itemize}
\end{block}

\vspace{0.15cm}

\begin{block}{Összegzés}
A bűnözési gyakoriság többféle szemléletből is eredményesen vizsgálható.
\end{block}
